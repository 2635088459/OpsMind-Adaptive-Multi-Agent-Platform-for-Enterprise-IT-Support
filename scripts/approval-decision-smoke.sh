...
token_for() {
  curl -sS -X POST "$KC/realms/$REALM/protocol/openid-connect/token" \
    ...
    -d "username=$1" -d password=test-password | jget "['access_token']"
}
...
status_of() { pg GET "/api/v1/approval-requests/$1" "$REQ_TOKEN" ""; jget "$BODY" "['status']"; }

# Create an ApprovalRequest. $1 = a label for logging. Echoes the approvalRequestId.
create_request() {
  local key hash src
...
print(json.dumps({
  ...
  # remember the linkage the decision must echo back
  LAST_SRC="$src"; LAST_HASH="$hash"
  jget "$BODY" "['approvalRequestId']"
}
...
say "6. event-relay consumed the decisions"
sleep 8
LOG="$(docker logs --since 2m "$RELAY_CONTAINER" 2>&1 || true)"
echo "$LOG" | grep -E "relay_consumed .*approval.(granted|denied).v1" | tail -6 | sed 's/^/   /' || true
if echo "$LOG" | grep -q "action=relay_consumed .*event_type=approval.granted.v1" \
   && echo "$LOG" | grep -q "action=relay_consumed .*event_type=approval.denied.v1"; then
  ok "relay consumed approval.granted.v1 + approval.denied.v1"
else
  die "event-relay did not consume both decision events (is opsmind-event-relay up?)"
fi

printf '\n\033[1;32mPASS\033[0m — ApprovalRequest granted (SoD) -> APPROVED, denied -> DENIED, both events published + relayed.\n'
