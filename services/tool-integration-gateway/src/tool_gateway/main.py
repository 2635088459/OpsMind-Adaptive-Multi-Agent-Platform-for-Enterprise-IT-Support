...
def _instrument_fastapi(app: FastAPI) -> None:
    ...
    continuation, so a tool request arriving from agent-runtime stays on the same
    trace and the manual domain spans (``tool.execute`` etc.) nest under it.
    Guarded — a missing optional package logs a warning, startup proceeds.
    """
    try:
        from opentelemetry.instrumentation.fastapi import FastAPIInstrumentor
    except ImportError:  # pragma: no cover - optional dependency
        logging.getLogger("tool_gateway").warning(
