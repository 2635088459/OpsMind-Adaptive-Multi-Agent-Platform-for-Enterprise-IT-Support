...
CAP_SEND_RESET_LINK = "identity.user.sendPasswordResetLink"

_REQUIRED_KEYS: dict[str, tuple[str, ...]] = {
    CAP_UNLOCK: ("username",),
    CAP_RESET_PASSWORD: ("username", "newPassword"),
    CAP_ADD_TO_GROUP: ("username", "groupName"),
    # userId (the Keycloak sub) OR username — resolved by _target_user_id below.
    CAP_SEND_RESET_LINK: (),
}
...
class KeycloakAdminConnectorAdapter(BaseConnector):
    ...
    _HANDLERS: dict[str, str] = {
        ...
        CAP_SEND_RESET_LINK: "_op_send_reset_link",
    }
    ...
    def _target_user_id(self, payload: dict[str, Any]) -> str:
        user_id = str(payload.get("userId", "")).strip()
        if user_id:
            return user_id
        return self._user_id(str(payload.get("username", "")))

    def _op_send_reset_link(self, payload: dict[str, Any]) -> tuple[str, dict[str, Any]]:
        """Keycloak sends the user a real password-reset email (execute-actions-email
        with UPDATE_PASSWORD). No secret is set here and none appears in the output;
        the reset link is single-use and generated by Keycloak. Needs realm SMTP
        configured — otherwise Keycloak returns 500 and this reports FAILED honestly.
        """
        user_id = self._target_user_id(payload)
        self._put(f"/admin/realms/{self._realm}/users/{user_id}/execute-actions-email", json=["UPDATE_PASSWORD"])
        return (
            f"sent a password-reset email to Keycloak user {user_id}",
            {"userId": user_id, "requiredAction": "UPDATE_PASSWORD", "channel": "email"},
        )
    ...
    def _put(self, path: str, json: dict[str, Any] | list[Any] | None = None) -> None:
