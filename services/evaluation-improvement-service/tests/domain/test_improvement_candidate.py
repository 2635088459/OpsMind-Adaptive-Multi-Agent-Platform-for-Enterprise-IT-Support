...
def _plan() -> CanaryPlan:
    return CanaryPlan("v1", (CanaryStage(5.0, 30, 0.05), CanaryStage(25.0, 30, 0.05)))
...
def test_full_happy_path_to_promoted() -> None:
    candidate = (
        _candidate().start_benchmarking(_NOW).record_benchmark_result(True, _NOW).request_approval(_NOW)
...
def test_failed_benchmark_can_be_rejected() -> None:
    candidate = _candidate().start_benchmarking(_NOW).record_benchmark_result(False, _NOW)
