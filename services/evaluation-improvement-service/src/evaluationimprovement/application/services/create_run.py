...
class CreateRunService:
    def __init__(
        self, dataset_repository: DatasetRepository, run_repository: EvaluationRunRepository, score_repository: ScoreRepository,
        langsmith_port: LangSmithPort, outbox_repository: OutboxRepository, audit_record_repository: AuditRecordRepository,
        clock: ClockPort,
    ) -> None:
    ...
    def create_run(self, command: CreateRunCommand) -> RunView:
        ...
        # 04-use-cases UC-EI-002 step 4: "07 收集 LangSmith experiment." Real mapping
        # logic is SPEC-EI-013 scope (infrastructure.langsmith's own module
        # docstrings); this call only proves the seam — a None return (today's only
        # possible outcome) means "unavailable," never a release-gate decision on its
        # own (10-failure-handling §"LangSmith 故障" is enforced downstream, not here).
    ...
    def find_scores(self, run_id: RunId) -> tuple[ScoreView, ...]:
        if self._run_repository.find_by_id(run_id) is None:
            raise RunNotFoundException(run_id)
        return tuple(score_to_view(s) for s in self._score_repository.find_active_by_run(run_id))
