...
class FakeAgentRuntimeEvaluationAdapter:
    def execute_case(self, run_id: RunId, target_version: str, test_case: EvaluationTestCase, run_generation: int) -> CaseExecutionResult:  # noqa: ARG002
        ...
        return CaseExecutionResult(
            ...
            cost_tokens=int(state.get("simulatedCostTokens", 100)),
            prompt_tokens=int(state.get("simulatedPromptTokens", int(state.get("simulatedCostTokens", 100) * 0.68))),
            completion_tokens=int(state.get("simulatedCompletionTokens", int(state.get("simulatedCostTokens", 100) * 0.32))),
            latency_ms=500, workflow_trace_ref=f"fake-trace-{run_id}",
    ...
    def execute_case(self, run_id: RunId, target_version: str, test_case: EvaluationTestCase, run_generation: int) -> CaseExecutionResult:
        ...
        try:
            ...
            return CaseExecutionResult(
                ...
                cost_tokens=int(body.get("costTokens", 0)),
                prompt_tokens=int(body.get("promptTokens", 0)), completion_tokens=int(body.get("completionTokens", 0)),
