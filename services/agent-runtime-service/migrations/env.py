...
from agentruntime.infrastructure.persistence.postgres.models import Base  # noqa: E402
from agentruntime.settings import get_settings  # noqa: E402
...
# 07-data-model §"Schema": all agent_runtime tables live under one dedicated
# schema — target_metadata drives `alembic revision --autogenerate` for future
# migrations; SPEC-ARO-002's initial migration is hand-written to match
# 07-data-model precisely (see migrations/versions/0001_*).
target_metadata = Base.metadata

# The DB URL comes from agentruntime.settings (DB_HOST/DB_PORT/DB_NAME/
# DB_USERNAME/DB_PASSWORD env vars) by default — same convention as the
# sibling ticket-workflow-service's Flyway config — never a real URL
# committed to alembic.ini (it only holds a dummy placeholder). Tests that
# invoke `alembic.command.upgrade()` programmatically against an ephemeral
# testcontainers database set a real "postgresql..." URL on the Config
# object *before* calling upgrade(); that already-configured value must win
# over the env-var default, or every programmatic run would silently
# migrate the wrong (dev) database instead of the one under test.
