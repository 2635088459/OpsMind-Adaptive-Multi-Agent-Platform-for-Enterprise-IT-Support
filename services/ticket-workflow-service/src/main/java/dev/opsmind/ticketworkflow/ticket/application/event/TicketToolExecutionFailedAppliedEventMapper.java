...
/** SPEC-TW-020 event-contract: maps {@link TicketToolExecutionFailedApplied} to a {@code ticket.tool-execution-failed-applied.v1} Outbox entry. */
@Component
public class TicketToolExecutionFailedAppliedEventMapper {

    private static final String AGGREGATE_TYPE = "Ticket";
    private static final String DATA_CLASSIFICATION = "INTERNAL";

    public OutboxEventEntry map(TicketToolExecutionFailedApplied event, SupportQueueId supportQueueId, String traceId, String correlationId, String causationId) {
        ...
        payload.put("safeToRetry", event.safeToRetry());

        return new OutboxEventEntry(
            UUID.randomUUID(),
            UUID.randomUUID().toString(),
            "ticket.tool-execution-failed-applied",
            "1.0",
