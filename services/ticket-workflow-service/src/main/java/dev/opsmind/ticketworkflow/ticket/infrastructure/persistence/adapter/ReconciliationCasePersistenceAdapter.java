...
/**
 * SPEC-TW-037 persistence: the guard read is a plain existence lookup on
 * {@code ticket.tickets} (mirrors {@code TicketAutoClosePersistenceAdapter},
 * SPEC-TW-027, without its status/version/resolution-cycle join — this SPEC
 * never inspects ticket lifecycle state). {@link #summarize} counts every
 * prior {@code ticket_phase10_open_reconciliation_case} attempt for the same
 * {@code (ticketId, sourceReference)} pair in one aggregate query, using
 * {@code FILTER} to also detect whether one is still open.
 */
@Component
public class ReconciliationCasePersistenceAdapter implements TicketReconciliationGuardPort, ReconciliationCaseRepository {
    ...
    private static final String SUMMARIZE_SQL = """
        ...
        FROM ticket.ticket_phase10_open_reconciliation_case
        WHERE ticket_id = :ticketId
          AND source_reference = :sourceReference
        """;

    private static final String INSERT_SQL = """
        INSERT INTO ticket.ticket_phase10_open_reconciliation_case (
    ...
    public ReconciliationCasePersistenceAdapter(JdbcTemplate jdbcTemplate, NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
        this.namedParameterJdbcTemplate = namedParameterJdbcTemplate;
    }

    @Override
    public Optional<TicketReconciliationGuard> loadGuard(TicketId ticketId) {
        List<TicketReconciliationGuard> rows = jdbcTemplate.query(
            GUARD_SQL,
            (rs, rowNum) -> {
                UUID supportQueueId = (UUID) rs.getObject("support_queue_id");
                return new TicketReconciliationGuard(
                    ticketId,
    ...
    @Override
    public ReconciliationCaseAttemptSummary summarize(TicketId ticketId, String sourceReference) {
        ...
        return namedParameterJdbcTemplate.queryForObject(SUMMARIZE_SQL, params, (rs, rowNum) ->
            new ReconciliationCaseAttemptSummary(rs.getInt("total_attempts"), rs.getInt("open_attempts") > 0)
        );
    }

    @Override
    public void record(ReconciliationCaseRecord record) {
        MapSqlParameterSource params = new MapSqlParameterSource()
