...
/** SPEC-TW-010 API contract §2: mirrors {@code TransitionTicketStatusController}'s header/version/idempotency handling. */
@RestController
public class ResolveTicketController {

    private static final int MAX_IDEMPOTENCY_KEY_LENGTH = 128;

    private final ResolveTicketUseCase useCase;
    private final ResolveTicketApiMapper mapper;

    public ResolveTicketController(ResolveTicketUseCase useCase, ResolveTicketApiMapper mapper) {
        this.useCase = useCase;
        this.mapper = mapper;
    }

    @PostMapping("/api/v1/tickets/{ticketId}/resolution")
    public ResponseEntity<ResolveTicketResponse> resolve(
        ...
        ResolveTicketCommand command = mapper.toCommand(
        ...
        ResolveTicketResult result = useCase.resolve(command);
