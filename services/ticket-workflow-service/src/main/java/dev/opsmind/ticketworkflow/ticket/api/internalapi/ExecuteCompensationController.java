...
/**
 * SPEC-TW-040 API contract: an internal (trusted-service-to-service), not
 ...
 * If-Match} ticket-version precondition: domain-rules "cannot run arbitrary
 * SQL or arbitrary state mutation" means this SPEC never mutates the Ticket
 * aggregate, so there is no ticket version to condition the request on.
 */
@RestController
public class ExecuteCompensationController {

    private static final int MAX_IDEMPOTENCY_KEY_LENGTH = 128;

    private final ExecuteCompensationUseCase useCase;
    private final ExecuteCompensationApiMapper mapper;

    public ExecuteCompensationController(ExecuteCompensationUseCase useCase, ExecuteCompensationApiMapper mapper) {
        this.useCase = useCase;
        this.mapper = mapper;
    }

    @PostMapping("/internal/v1/tickets/{ticketId}/compensations")
    public ResponseEntity<ExecuteCompensationResponse> execute(
        @PathVariable UUID ticketId,
        @Valid @RequestBody ExecuteCompensationRequest request,
        ...
        ExecuteCompensationCommand command = mapper.toCommand(
            TicketId.of(ticketId), request, actor,
            idempotencyKey, resolveCorrelationId(correlationId), UUID.randomUUID().toString(), Instant.now()
        );

        ExecuteCompensationResult result = useCase.execute(command);
        return ResponseEntity.ok()
            .location(URI.create("/internal/v1/tickets/" + ticketId + "/compensations/" + result.recoveryId()))
