...
/**
 * SPEC-TW-022 API contract: an internal (trusted-service-to-service),
 * not human-facing, endpoint — mirrors {@code RequestApprovalController}'s
 * header/version/idempotency handling exactly, but under the {@code
 * /internal/v1/} namespace and without any Support Queue/team resolution,
 * since the caller here is a trusted internal orchestrator, not an IT
 * Support agent (SPEC-TW-022 domain-rules §"clients cannot spoof
 * verification result" — {@code
 * StartVerificationApplicationService#authorize} enforces the {@code
 * SERVICE} actor type instead).
 */
@RestController
public class StartVerificationController {
    ...
    private final StartVerificationUseCase useCase;
    private final StartVerificationApiMapper mapper;

    public StartVerificationController(StartVerificationUseCase useCase, StartVerificationApiMapper mapper) {
        this.useCase = useCase;
        this.mapper = mapper;
    }

    @PostMapping("/internal/v1/tickets/{ticketId}/verification/start")
    public ResponseEntity<StartVerificationResponse> startVerification(
        ...
        StartVerificationCommand command = mapper.toCommand(
        ...
        StartVerificationResult result = useCase.startVerification(command);
        return ResponseEntity.status(HttpStatus.CREATED)
            .eTag(String.valueOf(result.version()))
            .location(URI.create("/internal/v1/tickets/" + ticketId + "/verification/attempts/" + result.verificationId()))
