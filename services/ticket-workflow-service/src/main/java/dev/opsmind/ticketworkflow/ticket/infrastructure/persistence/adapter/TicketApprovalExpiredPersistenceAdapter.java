...
/**
 * SPEC-TW-017 persistence: mirrors {@code TicketApprovalRejectedPersistenceAdapter}'s
 * shape (SPEC-TW-016). The guard read additionally selects the request's
 * own {@code expires_at} (needed for the Application layer's {@code
 * expiredAt >= expiresAt} business-rule check when populated) and {@code
 * action_type} (sourced from this row rather than the input event, since
 * CON-008 does not carry it). The write path updates the ticket row first,
 * guarded by {@code approval_reference = :approvalId} in addition to
 * status/version, then marks the approval request {@code EXPIRED}. A
 * ticket-row update failure (0 rows) means the guard-time state already
 * changed concurrently; the caller reclassifies that as {@code STALE}. An
 * approval-request update failure after the ticket update already
 * succeeded would mean the two rows disagree despite the guard query
 * having just observed them consistent — that violates this schema's own
 * invariants, so it is a fail-fast bug worth investigating rather than a
 * business outcome to swallow.
 */
@Component
public class TicketApprovalExpiredPersistenceAdapter implements TicketApprovalExpirationGuardPort, TicketApprovalExpiredRepository {

    private static final String GUARD_SQL = """
        SELECT ar.approval_request_id, ar.request_status, ar.workflow_id, ar.action_id, ar.action_type, ar.expires_at,
    ...
    private static final String UPDATE_APPROVAL_REQUEST_SQL = """
        ...
        SET request_status = 'EXPIRED',
            ...
            expired_event_id = :expiredEventId
    ...
    public TicketApprovalExpiredPersistenceAdapter(NamedParameterJdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    @Override
    public Optional<TicketApprovalExpirationGuard> loadGuard(TicketId ticketId, String approvalId) {
        ...
        return Optional.of(new TicketApprovalExpirationGuard(
            ticketId,
    ...
    @Override
    public TicketApprovalExpiredUpdateOutcome applyApprovalExpired(TicketApprovalExpiredUpdate update) {
        ...
        if (ticketRowsAffected != 1) {
            return new TicketApprovalExpiredUpdateOutcome.Conflict();
        }

        MapSqlParameterSource requestParams = new MapSqlParameterSource()
            .addValue("approvalRequestId", update.approvalRequestId())
            .addValue("expiredAt", Timestamp.from(update.expiredAt()))
            .addValue("expirationReason", update.expirationReason())
            .addValue("expiredEventId", update.expiredEventId());
