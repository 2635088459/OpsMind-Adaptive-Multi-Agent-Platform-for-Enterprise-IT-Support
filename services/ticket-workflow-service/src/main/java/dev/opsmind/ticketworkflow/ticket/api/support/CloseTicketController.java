...
/** SPEC-TW-011 API contract §1: mirrors {@code ResolveTicketController}'s header/version/idempotency handling. */
@RestController
public class CloseTicketController {
    ...
    private final CloseTicketApiMapper mapper;

    public CloseTicketController(CloseTicketUseCase useCase, CloseTicketApiMapper mapper) {
        this.useCase = useCase;
        this.mapper = mapper;
    }

    @PostMapping("/api/v1/tickets/{ticketId}/closure")
    public ResponseEntity<CloseTicketResponse> close(
        ...
        CloseTicketCommand command = mapper.toCommand(
        ...
        CloseTicketResult result = useCase.close(command);
