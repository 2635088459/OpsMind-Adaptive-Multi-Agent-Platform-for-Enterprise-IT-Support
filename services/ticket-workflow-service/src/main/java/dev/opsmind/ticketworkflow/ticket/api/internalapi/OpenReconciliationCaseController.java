...
/**
 * SPEC-TW-037 API contract: an internal (trusted-service-to-service), not
 * human-facing, endpoint — mirrors {@code AutoCloseTicketController}'s
 * (SPEC-TW-027) header/version/idempotency handling, minus the {@code
 * If-Match} ticket-version precondition: domain-rules "must not directly
 * repair business state" means this SPEC never mutates the Ticket aggregate,
 * so there is no ticket version to condition the request on.
 */
@RestController
public class OpenReconciliationCaseController {

    private static final int MAX_IDEMPOTENCY_KEY_LENGTH = 128;

    private final OpenReconciliationCaseUseCase useCase;
    private final OpenReconciliationCaseApiMapper mapper;

    public OpenReconciliationCaseController(OpenReconciliationCaseUseCase useCase, OpenReconciliationCaseApiMapper mapper) {
        this.useCase = useCase;
        this.mapper = mapper;
    }

    @PostMapping("/internal/v1/tickets/{ticketId}/reconciliation-cases")
    public ResponseEntity<OpenReconciliationCaseResponse> openCase(
        @PathVariable UUID ticketId,
        @Valid @RequestBody OpenReconciliationCaseRequest request,
        ...
        OpenReconciliationCaseCommand command = mapper.toCommand(
            TicketId.of(ticketId), request, actor,
            idempotencyKey, resolveCorrelationId(correlationId), UUID.randomUUID().toString(), Instant.now()
        );

        OpenReconciliationCaseResult result = useCase.openCase(command);
        return ResponseEntity.ok()
            .location(URI.create("/internal/v1/tickets/" + ticketId + "/reconciliation-cases/" + result.recoveryId()))
