...
/**
 * SPEC-TW-038 API contract: an internal (trusted-service-to-service), not
 * human-facing, endpoint — mirrors {@code OpenReconciliationCaseController}'s
 * (SPEC-TW-037) header/idempotency handling. Unlike that endpoint, this one
 * has no {@code {ticketId}} path variable — {@code
 * /internal/v1/tickets/events/replay} is not ticket-scoped, since the
 * recovery target is an original event, not a ticket directly.
 */
@RestController
public class ReplayEventController {

    private static final int MAX_IDEMPOTENCY_KEY_LENGTH = 128;

    private final ReplayEventUseCase useCase;
    private final ReplayEventApiMapper mapper;

    public ReplayEventController(ReplayEventUseCase useCase, ReplayEventApiMapper mapper) {
        this.useCase = useCase;
        this.mapper = mapper;
    }

    @PostMapping("/internal/v1/tickets/events/replay")
    public ResponseEntity<ReplayEventResponse> replay(
        @Valid @RequestBody ReplayEventRequest request,
        ...
        ReplayEventCommand command = mapper.toCommand(
            request, actor, idempotencyKey, resolveCorrelationId(correlationId), UUID.randomUUID().toString(), Instant.now()
        );

        ReplayEventResult result = useCase.replay(command);
        return ResponseEntity.ok()
            .location(URI.create("/internal/v1/tickets/events/replay/" + result.recoveryId()))
