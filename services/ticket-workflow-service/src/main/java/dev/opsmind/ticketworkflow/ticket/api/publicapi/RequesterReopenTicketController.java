...
/**
 * SPEC-TW-028 API contract: {@code RESOLVED|CLOSED -> IN_PROGRESS} through
 * requester or authorized-support reopen. Mirrors {@code
 * ConfirmResolutionController}'s (SPEC-TW-026) actor derivation — the
 * primary caller is the ticket's own requester — under a distinct path from
 * {@code ReopenTicketController}'s (SPEC-TW-011) {@code /reopen}, since that
 * route is already claimed by the IT-support-only endpoint.
 */
@RestController
public class RequesterReopenTicketController {

    private static final int MAX_IDEMPOTENCY_KEY_LENGTH = 128;

    private final RequesterReopenTicketUseCase useCase;
    private final RequesterReopenTicketApiMapper mapper;

    public RequesterReopenTicketController(RequesterReopenTicketUseCase useCase, RequesterReopenTicketApiMapper mapper) {
        this.useCase = useCase;
        this.mapper = mapper;
    }

    @PostMapping("/api/v1/tickets/{ticketId}/reopen-request")
    public ResponseEntity<RequesterReopenTicketResponse> reopen(
        @PathVariable UUID ticketId,
        @Valid @RequestBody RequesterReopenTicketRequest request,
        ...
        RequesterReopenTicketCommand command = mapper.toCommand(
