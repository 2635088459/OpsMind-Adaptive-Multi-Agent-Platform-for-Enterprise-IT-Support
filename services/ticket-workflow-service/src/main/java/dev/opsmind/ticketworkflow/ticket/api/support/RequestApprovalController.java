...
/** SPEC-TW-014 API contract: mirrors {@code RequestUserInputController}'s header/version/idempotency handling, and returns {@code 201 Created}. */
@RestController
public class RequestApprovalController {
    ...
    private final RequestApprovalApiMapper mapper;

    public RequestApprovalController(RequestApprovalUseCase useCase, RequestApprovalApiMapper mapper) {
        this.useCase = useCase;
        this.mapper = mapper;
    }

    @PostMapping("/api/v1/tickets/{ticketId}/approval-requests")
    public ResponseEntity<RequestApprovalResponse> requestApproval(
        ...
        RequestApprovalCommand command = mapper.toCommand(
        ...
        RequestApprovalResult result = useCase.requestApproval(command);
        return ResponseEntity.status(HttpStatus.CREATED)
            .eTag(String.valueOf(result.version()))
            .location(URI.create("/api/v1/tickets/" + ticketId + "/approval-requests/" + result.approvalRequestId()))
