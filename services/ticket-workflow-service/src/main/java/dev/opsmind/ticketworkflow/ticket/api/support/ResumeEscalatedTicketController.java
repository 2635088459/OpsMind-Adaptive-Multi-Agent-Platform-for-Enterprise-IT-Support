...
/**
 * SPEC-TW-032 API contract: {@code POST
 * /api/v1/tickets/{ticketId}/escalation/resume} — {@code ESCALATED ->
 * IN_PROGRESS}, for a support lead or escalation owner (domain-rules).
 * Mirrors {@code EscalateTicketController}'s (SPEC-TW-031) boilerplate.
 */
@RestController
public class ResumeEscalatedTicketController {

    private static final int MAX_IDEMPOTENCY_KEY_LENGTH = 128;

    private final ResumeEscalatedTicketUseCase useCase;
    private final ResumeEscalatedTicketApiMapper mapper;

    public ResumeEscalatedTicketController(ResumeEscalatedTicketUseCase useCase, ResumeEscalatedTicketApiMapper mapper) {
        this.useCase = useCase;
        this.mapper = mapper;
    }

    @PostMapping("/api/v1/tickets/{ticketId}/escalation/resume")
    public ResponseEntity<ResumeEscalatedTicketResponse> resume(
        @PathVariable UUID ticketId,
        @Valid @RequestBody ResumeEscalatedTicketRequest request,
        ...
        ResumeEscalatedTicketCommand command = mapper.toCommand(
        ...
        ResumeEscalatedTicketResult result = useCase.resume(command);
