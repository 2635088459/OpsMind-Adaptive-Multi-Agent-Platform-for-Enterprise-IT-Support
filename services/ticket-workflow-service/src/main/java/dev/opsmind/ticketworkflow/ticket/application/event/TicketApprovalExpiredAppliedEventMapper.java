...
/** SPEC-TW-017 event-contract: maps {@link TicketApprovalExpiredApplied} to a {@code ticket.approval-expired-applied.v1} Outbox entry. */
@Component
public class TicketApprovalExpiredAppliedEventMapper {

    private static final String AGGREGATE_TYPE = "Ticket";
    private static final String DATA_CLASSIFICATION = "INTERNAL";

    public OutboxEventEntry map(TicketApprovalExpiredApplied event, SupportQueueId supportQueueId, String traceId, String correlationId, String causationId) {
        ...
        payload.put("expiredAt", event.expiredAt().toString());

        return new OutboxEventEntry(
            UUID.randomUUID(),
            UUID.randomUUID().toString(),
            "ticket.approval-expired-applied",
            "1.0",
