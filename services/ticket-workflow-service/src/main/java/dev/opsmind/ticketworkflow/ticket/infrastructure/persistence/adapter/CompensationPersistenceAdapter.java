...
/**
 * SPEC-TW-040 persistence: the guard read is a plain existence lookup on
 * {@code ticket.tickets} (mirrors {@code ReconciliationCasePersistenceAdapter},
 * SPEC-TW-037). {@link #summarize} counts every prior {@code
 * ticket_phase10_compensation} attempt for the same {@code (ticketId,
 * sourceReference)} pair in one aggregate query, using {@code FILTER} to
 * also detect whether one is still open.
 */
@Component
public class CompensationPersistenceAdapter implements TicketCompensationGuardPort, CompensationRepository {
    ...
    private static final String SUMMARIZE_SQL = """
        ...
        FROM ticket.ticket_phase10_compensation
        WHERE ticket_id = :ticketId
          AND source_reference = :sourceReference
        """;

    private static final String INSERT_SQL = """
        INSERT INTO ticket.ticket_phase10_compensation (
            id, ticket_id, compensation_action, source_reference, decision, reason_code, reason,
            actor_id, correlation_id, causation_id, attempt_number, created_at, completed_at
        ) VALUES (
            :id, :ticketId, :compensationAction, :sourceReference, :decision, :reasonCode, :reason,
    ...
    public CompensationPersistenceAdapter(JdbcTemplate jdbcTemplate, NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
        this.namedParameterJdbcTemplate = namedParameterJdbcTemplate;
    }

    @Override
    public Optional<TicketCompensationGuard> loadGuard(TicketId ticketId) {
        List<TicketCompensationGuard> rows = jdbcTemplate.query(
            GUARD_SQL,
            (rs, rowNum) -> {
                UUID supportQueueId = (UUID) rs.getObject("support_queue_id");
                return new TicketCompensationGuard(
                    ticketId,
    ...
    @Override
    public CompensationAttemptSummary summarize(TicketId ticketId, String sourceReference) {
        ...
        return namedParameterJdbcTemplate.queryForObject(SUMMARIZE_SQL, params, (rs, rowNum) ->
            new CompensationAttemptSummary(rs.getInt("total_attempts"), rs.getInt("open_attempts") > 0)
        );
    }
    ...
    public void record(CompensationRecord record) {
        MapSqlParameterSource params = new MapSqlParameterSource()
            ...
            .addValue("compensationAction", record.compensationAction().name())
