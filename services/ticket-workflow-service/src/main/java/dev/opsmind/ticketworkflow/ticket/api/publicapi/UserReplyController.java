...
/** SPEC-TW-013 API contract: mirrors {@code PublicTicketMessageController}'s actor derivation, plus the {@code If-Match} handling every ticket-write endpoint uses. */
@RestController
public class UserReplyController {

    private static final int MAX_IDEMPOTENCY_KEY_LENGTH = 128;

    private final UserReplyAndResumeUseCase useCase;
    private final UserReplyApiMapper mapper;

    public UserReplyController(UserReplyAndResumeUseCase useCase, UserReplyApiMapper mapper) {
        this.useCase = useCase;
        this.mapper = mapper;
    }

    @PostMapping("/api/v1/tickets/{ticketId}/user-input-requests/{requestId}/reply")
    public ResponseEntity<UserReplyResponse> reply(
        @PathVariable UUID ticketId,
        @PathVariable UUID requestId,
        @Valid @RequestBody UserReplyRequest request,
        ...
        UserReplyAndResumeCommand command = mapper.toCommand(
            TicketId.of(ticketId), requestId, request, actor, expectedVersion,
            idempotencyKey, resolveCorrelationId(correlationId), UUID.randomUUID().toString(), Instant.now()
        );

        UserReplyAndResumeResult result = useCase.reply(command);
