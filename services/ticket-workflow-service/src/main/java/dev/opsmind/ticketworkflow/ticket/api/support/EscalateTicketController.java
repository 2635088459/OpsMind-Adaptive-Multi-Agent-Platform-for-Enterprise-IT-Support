...
/**
 * SPEC-TW-031 API contract: {@code POST /api/v1/tickets/{ticketId}/escalation}
 * — {@code mutable non-terminal state -> ESCALATED}, for a support actor,
 * policy worker, or failure handler (domain-rules), never a requester
 * (unlike Cancel/Confirm-Resolution/Requester-Reopen). Mirrors {@code
 * UpdateTicketAssignmentController}'s (SPEC-TW-030) boilerplate.
 */
@RestController
public class EscalateTicketController {
    ...
    private final EscalateTicketUseCase useCase;
    private final EscalateTicketApiMapper mapper;

    public EscalateTicketController(EscalateTicketUseCase useCase, EscalateTicketApiMapper mapper) {
        this.useCase = useCase;
        this.mapper = mapper;
    }

    @PostMapping("/api/v1/tickets/{ticketId}/escalation")
    public ResponseEntity<EscalateTicketResponse> escalate(
        ...
        EscalateTicketCommand command = mapper.toCommand(
        ...
        EscalateTicketResult result = useCase.escalate(command);
