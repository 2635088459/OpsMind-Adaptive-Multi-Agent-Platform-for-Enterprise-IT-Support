...
/**
 * SPEC-TW-036 api-contract: {@code POST /internal/v1/security/step-up/evaluate}.
 * An internal (trusted-service-to-service), not human-facing, endpoint
 * under the same {@code /internal/v1/} namespace and JWT-principal-
 * resolution shape as {@code SupportQueueAuthorizationController}/{@code
 * SensitiveReadAuditPolicyController}/{@code SecretDetectionController} —
 * the caller's own admission is enforced inside {@code
 * StepUpAuthenticationPolicyApplicationService}, not here, so every entry
 * point into the policy (controller, application service, inline call from
 * Cancel/Escalate) shares one admission rule.
 */
@RestController
public class StepUpAuthenticationController {

    private final EvaluateStepUpAuthenticationUseCase useCase;
    private final StepUpAuthenticationEvaluateApiMapper mapper;

    public StepUpAuthenticationController(EvaluateStepUpAuthenticationUseCase useCase, StepUpAuthenticationEvaluateApiMapper mapper) {
        this.useCase = useCase;
        this.mapper = mapper;
    }

    @PostMapping("/internal/v1/security/step-up/evaluate")
    public ResponseEntity<StepUpAuthenticationEvaluateResponse> evaluate(
        @Valid @RequestBody StepUpAuthenticationEvaluateRequest request,
        @RequestHeader(value = "X-Correlation-Id", required = false) String correlationId,
        @AuthenticationPrincipal Jwt jwt
    ) {
        ActorContext caller = actorFrom(jwt);
        EvaluateStepUpAuthenticationCommand command = mapper.toCommand(
            request, caller, resolveCorrelationId(correlationId), currentTraceId()
        );
        EvaluateStepUpAuthenticationResult result = useCase.evaluate(command);
