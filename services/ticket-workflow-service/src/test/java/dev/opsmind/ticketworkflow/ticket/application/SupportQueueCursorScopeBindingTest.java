...
/**
 * SPEC-TW-005 §14: a cursor is bound to the authorization scope active
 * when it was issued — acceptance scenario "Cursor cannot be reused after
 * scope changes" (e.g. the actor's authorized team set changes between
 * pages).
 */
@Tag("unit")
class SupportQueueCursorScopeBindingTest {
    ...
    private SupportQueueCursor cursorFor(String scopeFingerprint) {
        return new SupportQueueCursor(
            SupportQueueCursor.CURRENT_VERSION, NOW, 0, 0, NOW.minusSeconds(3600), UUID.randomUUID(),
            "sha256:filters", scopeFingerprint, "support-100", SupportQueueSortVersion.CURRENT_VERSION,
    ...
    void shouldAcceptWhenScopeFingerprintMatches() {
        SupportQueueCursor cursor = cursorFor("sha256:scope-before-change");

        assertThatCode(() -> codec.requireMatch(cursor, "sha256:filters", "sha256:scope-before-change", "support-100"))
    ...
    void shouldRejectWhenScopeFingerprintDiffersAfterAPermissionChange() {
        ...
        assertThatThrownBy(() -> codec.requireMatch(cursor, "sha256:filters", "sha256:scope-after-change", "support-100"))
