...
/**
 * SPEC-TW-015 persistence/E2E: full Spring context + real PostgreSQL and
 * RabbitMQ Testcontainers, publishing a real {@code approval.granted.v1}
 * message and letting the real {@code @RabbitListener} consume it. Mirrors
 * {@code TicketRequestApprovalIT}'s Postgres-seeding style, but drives the
 * scenario through the broker instead of {@code TestRestTemplate} since
 * SPEC-TW-015 has no HTTP endpoint.
 */
@Tag("integration")
class TicketApplyApprovalGrantedIT extends AbstractTicketAssignmentIT {
    ...
    private String grantedPayload(String workflowId, String actionId, String actionType, String approvalId, Instant approvedAt, Instant expiresAt) {
        return """
            {
              "workflowId": "%s",
              "actionId": "%s",
            ...
            """.formatted(workflowId, actionId, actionType, approvalId, approvedAt, expiresAt);
    }

    private void publish(String body) {
        rabbitTemplate.convertAndSend(RabbitMqConfiguration.EVENTS_EXCHANGE, "approval.granted.v1", body);
    }

    @Test
    void shouldApplyApprovalGrantedAndTransitionTheTicketBackToInProgress() {
        ...
        Instant approvedAt = Instant.parse("2026-08-03T17:50:00Z");
        Instant expiresAt = Instant.parse("2026-08-03T18:30:00Z");
        publish(envelope("evt-granted-1", ALLOWED_PRODUCER, ticketId.toString(), grantedPayload("wf-9000", "act-100", "RESET_MFA", approvalId, approvedAt, expiresAt)));
        ...
        assertThat(requestRow.get("approved_by")).isEqualTo("sha256:approver");
        assertThat(requestRow.get("authorization_reference")).isNotNull();
        assertThat(requestRow.get("granted_event_id")).isEqualTo("evt-granted-1");

        Integer historyCount = jdbcTemplate.queryForObject(
            "SELECT count(*) FROM ticket.ticket_status_history WHERE ticket_id = ? AND transition_id = 'SM-017'", Integer.class, ticketId
        );
        assertThat(historyCount).isEqualTo(1);

        Integer outboxCount = jdbcTemplate.queryForObject(
            "SELECT count(*) FROM ticket.outbox_events WHERE ticket_id = ? AND event_type = 'ticket.approval-granted-applied'", Integer.class, ticketId
        );
    ...
    void shouldBeIdempotentOnADuplicateDelivery() {
        ...
        String body = envelope(
            "evt-granted-dup", ALLOWED_PRODUCER, ticketId.toString(),
            grantedPayload("wf-9000", "act-100", "RESET_MFA", approvalId, Instant.parse("2026-08-03T17:50:00Z"), Instant.parse("2026-08-03T18:30:00Z"))
        );
    ...
    void shouldSendASchemaInvalidMessageToTheDeadLetterQueue() {
        ...
        String invalidPayload = """
            {"workflowId": "wf-9000", "actionId": "act-100", "actionType": "RESET_MFA"}
            """;
    ...
    void shouldAckAReferenceMismatchAsStaleWithoutAdvancingOrDeadLettering() {
        ...
        publish(envelope(
            ...
            grantedPayload("wf-DIFFERENT", "act-100", "RESET_MFA", approvalId, Instant.parse("2026-08-03T17:50:00Z"), Instant.parse("2026-08-03T18:30:00Z"))
        ));
