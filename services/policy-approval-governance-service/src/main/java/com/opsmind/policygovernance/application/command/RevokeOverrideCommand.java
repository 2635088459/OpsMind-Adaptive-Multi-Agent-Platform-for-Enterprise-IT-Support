...
/**
 * Input to {@code ApprovalService.revoke} (04-use-cases §UC-PG-006,
 * SPEC-PG-022). Mirrors {@link CancelApprovalCommand}'s shape: {@code
 * sourceRequestId}/{@code requestHash} re-validate request linkage
 * (INV-PG-005) and {@code commandIdempotencyKey} is this command's own
 * idempotency guard — see {@code
 * domain.approval.ApprovalRequest#revokedCommandIdempotencyKey}.
 *
 * <p>{@code causationId} is SPEC-PG-029's own addition — see {@link
 * DecideApprovalCommand#causationId()}'s own javadoc for the same
 * reasoning.
 */
public record RevokeOverrideCommand(
