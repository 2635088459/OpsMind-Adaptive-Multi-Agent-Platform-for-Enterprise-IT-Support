...
- `WAITING_FOR_USER`：`waitingForRequesterSince` 必须非空，`approvalReference` 必须为空。
- `WAITING_FOR_APPROVAL`：`approvalReference` 必须非空，`waitingForRequesterSince` 必须为空。
- `IN_PROGRESS`：两个 waiting metadata 字段都必须为空。
...
事件不得包含 token、email、原始 claims、私密 Ticket message、审批详情正文、队列成员证明或完整身份资料。
...
Outbox 与 publisher telemetry 包含 event type、event ID、aggregate ID/version、correlation ID、attempt count 和 outcome。Metric label 不包含 reason、actor ID 或 idempotency key。
