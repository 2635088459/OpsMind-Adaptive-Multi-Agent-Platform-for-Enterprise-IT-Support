# SPEC-ARO-020 — Consume tool.completed and tool.failed（消费 tool.completed 与 tool.failed）
...
>
> LLD 映射：`04-use-cases`, `06-event-contracts`
>
> 文档状态：Spec Planning

## 1. 目标

实现 `消费 tool.completed 与 tool.failed`，并保持 Runtime 只拥有 Agent Workflow state，不直接修改 Ticket state。
