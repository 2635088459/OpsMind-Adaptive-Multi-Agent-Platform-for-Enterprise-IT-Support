# SPEC-ARO-009 — Task Claim Lease Worker API（Task Claim Lease Worker API）
...
实现 `Task Claim Lease Worker API`，并保持 Runtime 只拥有 Agent Workflow state，不直接修改 Ticket state。
