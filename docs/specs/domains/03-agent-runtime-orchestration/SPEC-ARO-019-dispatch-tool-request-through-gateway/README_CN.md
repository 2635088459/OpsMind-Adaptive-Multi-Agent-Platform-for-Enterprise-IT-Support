# SPEC-ARO-019 — Dispatch Tool Request Through Gateway（通过 Tool Gateway 派发 Tool Request）

> 领域：Agent Runtime Orchestration
>
> Phase：05 — Tool Gateway 中介
>
> 服务：`agent-runtime-service`
>
> LLD 映射：`08-transaction-and-outbox`, `11-security`
>
> 文档状态：Spec Planning

## 1. 目标

实现 `通过 Tool Gateway 派发 Tool Request`，并保持 Runtime 只拥有 Agent Workflow state，不直接修改 Ticket state。
