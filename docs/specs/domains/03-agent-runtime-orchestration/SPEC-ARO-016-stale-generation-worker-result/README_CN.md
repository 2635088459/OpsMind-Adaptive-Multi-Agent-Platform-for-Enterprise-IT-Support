# SPEC-ARO-016 — Stale Generation Worker Result（旧 Generation Worker Result 防护）

> 领域：Agent Runtime Orchestration
>
> Phase：04 — Pause Resume 控制
>
> 服务：`agent-runtime-service`
>
> LLD 映射：`02-business-invariants`, `10-failure-handling`
>
> 文档状态：Spec Planning

## 1. 目标

实现 `旧 Generation Worker Result 防护`，并保持 Runtime 只拥有 Agent Workflow state，不直接修改 Ticket state。
