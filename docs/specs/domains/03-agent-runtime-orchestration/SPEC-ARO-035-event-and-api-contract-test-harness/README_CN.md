# SPEC-ARO-035 — Event and API Contract Test Harness（Event 与 API 契约测试框架）
...
>
> LLD 映射：`14-testing-strategy`, `06-event-contracts`
>
> 文档状态：Spec Planning

## 1. 目标

实现 `Event 与 API 契约测试框架`，并保持 Runtime 只拥有 Agent Workflow state，不直接修改 Ticket state。
