# SPEC-ARO-012 — Checkpoint Around External Waits（外部等待前后的 Checkpoint）
...
>
> LLD 映射：`03-state-machine`, `08-transaction-and-outbox`
>
> 文档状态：Spec Planning

## 1. 目标

实现 `外部等待前后的 Checkpoint`，并保持 Runtime 只拥有 Agent Workflow state，不直接修改 Ticket state。
