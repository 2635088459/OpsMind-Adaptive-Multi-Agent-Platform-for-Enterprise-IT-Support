# SPEC-ARO-005 — Start Workflow From ticket.created（从 ticket.created 启动 Workflow）

> 领域：Agent Runtime Orchestration
>
> Phase：01 — Workflow Instance 生命周期
>
...
实现 `从 ticket.created 启动 Workflow`，并保持 Runtime 只拥有 Agent Workflow state，不直接修改 Ticket state。
