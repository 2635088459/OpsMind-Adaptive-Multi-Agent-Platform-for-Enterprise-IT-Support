# Phase 03 — 执行 Worker 与 Connector
...
实现执行调度、worker claim/lease、connector SDK、凭据准备、operation key、副作用防护、结果标准化与 tool.completed 发布。
...
| 2 | `SPEC-TG-011` | Connector SDK 与内置 Fake Connector | 13-package-and-class-design, 14-testing-strategy |
| 3 | `SPEC-TG-012` | Credential Binding 与调用准备 | 01-domain-model, 11-security |
| 4 | `SPEC-TG-013` | Operation Key 与副作用防护 | 09-concurrency-and-idempotency, 10-failure-handling |
| 5 | `SPEC-TG-014` | Result Envelope 标准化与脱敏 | 01-domain-model, 11-security, 05-api-contracts |
| 6 | `SPEC-TG-015` | tool.completed 事件发布 | 06-event-contracts, 08-transaction-and-outbox |
